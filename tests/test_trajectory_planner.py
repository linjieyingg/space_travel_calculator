...
def test_plan_hohmann_trajectory_success_earth_mars(mock_celestial_data_for_planner, mock_orbital_mechanics):
    """
    Test successful planning of an Earth to Mars Hohmann transfer.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("Earth", "Mars")
...
def test_plan_hohmann_trajectory_input_validation_empty_departure(mock_celestial_data_for_planner):
    """
    Test planning with an empty departure body name.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("", "Mars")
    assert result["success"] is False
    assert "non-empty string" in result["error"]

def test_plan_hohmann_trajectory_input_validation_whitespace_arrival(mock_celestial_data_for_planner):
    """
    Test planning with a whitespace-only arrival body name.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("Earth", "  ")
    assert result["success"] is False
    assert "non-empty string" in result["error"]

def test_plan_hohmann_trajectory_input_validation_non_string_departure(mock_celestial_data_for_planner):
    """
    Test planning with a non-string departure body name.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory(123, "Mars")
    assert result["success"] is False
    assert "non-empty string" in result["error"]

def test_plan_hohmann_trajectory_same_departure_arrival_bodies(mock_celestial_data_for_planner):
    """
    Test planning where departure and arrival bodies are the same (case-insensitive).
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("Earth", "earth")
    assert result["success"] is False
    assert "cannot be the same" in result["error"]

def test_plan_hohmann_trajectory_non_existent_departure_body(mock_celestial_data_for_planner):
    """
    Test planning with a non-existent departure body.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("NonExistentPlanet", "Mars")
    assert result["success"] is False
    assert "Could not find data for departure body" in result["error"]

def test_plan_hohmann_trajectory_non_existent_arrival_body(mock_celestial_data_for_planner):
    """
    Test planning with a non-existent arrival body.
    """
    planner = TrajectoryPlanner()
    result = planner.plan_hohmann_trajectory("Earth", "NonExistentPlanet")
    assert result["success"] is False
    assert "Could not find data for arrival body" in result["error"]

def test_plan_hohmann_trajectory_missing_semi_major_axis_departure(mock_celestial_data_for_planner):
    """
    Test planning when 'semi_major_axis_from_sun' is missing for the departure body.
    """
...
def test_plan_hohmann_trajectory_invalid_semi_major_axis_arrival(mock_celestial_data_for_planner):
    """
    Test planning when 'semi_major_axis_from_sun' is invalid for the arrival body.
    """
    mock_celestial_data_for_planner.side_effect = lambda name: {
        'sun': {'name': 'Sun', 'gravitational_parameter_mu': 1.327e20},
...
def test_plan_hohmann_trajectory_orbital_mechanics_value_error(mock_celestial_data_for_planner, mock_orbital_mechanics):
    """
    Test planning when orbital_mechanics functions raise a ValueError.
    """
...
def test_plan_hohmann_trajectory_orbital_mechanics_unexpected_error(mock_celestial_data_for_planner, mock_orbital_mechanics):
    """
    Test planning when orbital_mechanics functions raise an unexpected error.
    """
...
def test_plan_hohmann_trajectory_with_fractional_travel_time(mock_celestial_data_for_planner, mock_orbital_mechanics):
    """
    Test planning with fractional travel time to ensure correct H:M:S formatting.
    """
