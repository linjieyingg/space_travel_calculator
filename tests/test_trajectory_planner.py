...
def mock_orbital_mechanics():
    with patch('trajectory_planner.orbital_mechanics.calculate_hohmann_transfer_delta_v') as mock_hohmann_delta_v, \
         patch('trajectory_planner.orbital_mechanics.calculate_hohmann_transfer_time_of_flight') as mock_hohmann_time_of_flight, \
         patch('trajectory_planner.orbital_mechanics.calculate_lambert_transfer') as mock_lambert_transfer: # NEW MOCK
        
        mock_hohmann_delta_v.return_value = (1000.0, 2000.0, 3000.0) # Delta-V1, Delta-V2, Total Delta-V
        mock_hohmann_time_of_flight.return_value = 100 * 24 * 3600 # 100 days in seconds
        
        # For Lambert, assume it returns (total_delta_v, time_of_flight_seconds)
        mock_lambert_transfer.return_value = (5000.0, 50 * 24 * 3600) # NEW RETURN VALUE for Lambert (delta_v, time_of_flight_seconds)
        
        # Yield all mocked objects
        yield mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer

# New Fixture for mocking ephemeris.get_heliocentric_state
@pytest.fixture
def mock_ephemeris_get_heliocentric_state():
    with patch('trajectory_planner.ephemeris.get_heliocentric_state') as mock_get_state:
        # Default behavior: return distinct, valid distances and angular positions for Earth and Mars
        def side_effect_ephemeris(body_name: str, date: datetime):
            # Simulate distinct positions for testing direct transfer which might care about alignment
            # Using specific angles to ensure delta_theta is not zero
            if body_name.lower() == 'earth':
                return {'distance_from_sun_m': 1.500e11, 'angular_position_rad': 0.0}
            elif body_name.lower() == 'mars':
                return {'distance_from_sun_m': 2.300e11, 'angular_position_rad': math.pi / 2} # 90 degrees
            return None # For other bodies or if no specific state is defined
        mock_get_state.side_effect = side_effect_ephemeris
        yield mock_get_state

# Helper to convert date string to datetime object as `plan_trajectory` expects string
...
def test_plan_trajectory_hohmann_success_earth_mars_with_ephemeris(
    mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state
):
    """
    ...
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
    ...
    mock_ephemeris_get_heliocentric_state.assert_has_calls([
        call('Earth', departure_date_obj),
        call('Mars', departure_date_obj)
    ], any_order=True)

    # Verify orbital mechanics functions were called with the distances from ephemeris
    ephemeris_earth_distance = mock_ephemeris_get_heliocentric_state.side_effect('Earth', departure_date_obj)['distance_from_sun_m']
    ephemeris_mars_distance = mock_ephemeris_get_heliocentric_state.side_effect('Mars', departure_date_obj)['distance_from_sun_m']

    mock_hohmann_delta_v.assert_called_once_with(
        planner.mu_sun,
        ephemeris_earth_distance,
        ephemeris_mars_distance
    )
    mock_hohmann_time_of_flight.assert_called_once_with(
        planner.mu_sun,
        ephemeris_earth_distance,
        ephemeris_mars_distance
    )
    # Ensure Lambert mock was NOT called
    mock_lambert_transfer.assert_not_called()
...
def test_plan_trajectory_hohmann_input_validation_empty_departure_with_date(mock_celestial_data_for_planner):
    ...
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    result = planner.plan_trajectory("", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
...
def test_plan_trajectory_hohmann_input_validation_whitespace_arrival_with_date(mock_celestial_data_for_planner):
    ...
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    result = planner.plan_trajectory("Earth", "  ", trajectory_type="Hohmann", departure_date=departure_date_str)
    assert result["success"] is False
    assert "non-empty string" in result["error"]
...
def test_plan_trajectory_hohmann_input_validation_non_string_departure_with_date(mock_celestial_data_for_planner):
    ...
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    result = planner.plan_trajectory(123, "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
    assert result["success"] is False
    assert "non-empty string" in result["error"]

def test_plan_trajectory_hohmann_same_departure_arrival_bodies_with_date(mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state):
    """
    ...
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    result = planner.plan_trajectory("Earth", "earth", trajectory_type="Hohmann", departure_date=departure_date_str)
    assert result["success"] is False
    assert "cannot be the same" in result["error"]
    # Ephemeris will be called for the departure body before the same-body check
    mock_ephemeris_get_heliocentric_state.assert_called_once_with('Earth', departure_date_obj)
...
def test_plan_trajectory_hohmann_ephemeris_fails_departure_fallback_succeeds(
    ...
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        None if name.lower() == 'earth' else {'distance_from_sun_m': 2.30e11, 'angular_position_rad': math.pi / 2} if name.lower() == 'mars' else None
...
def test_plan_trajectory_hohmann_ephemeris_fails_arrival_fallback_succeeds(
    ...
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 1.50e11, 'angular_position_rad': 0.0} if name.lower() == 'earth' else None if name.lower() == 'mars' else None
...
def test_plan_trajectory_hohmann_ephemeris_and_static_fail_departure(
    mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state
):
    """
...
def test_plan_trajectory_hohmann_ephemeris_missing_distance_key_fallback_succeeds(
    ...
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'some_other_key': 123, 'angular_position_rad': 0.0} if name.lower() == 'earth' else {'distance_from_sun_m': 2.30e11, 'angular_position_rad': math.pi / 2} if name.lower() == 'mars' else None
    ...
    mock_hohmann_delta_v.assert_called_once_with(planner.mu_sun, static_earth_distance, ephemeris_mars_distance)
    mock_hohmann_time_of_flight.assert_called_once_with(planner.mu_sun, static_earth_distance, ephemeris_mars_distance)
    mock_lambert_transfer.assert_not_called()
...
def test_plan_trajectory_hohmann_ephemeris_invalid_distance_value_fallback_succeeds(
    ...
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 1.50e11, 'angular_position_rad': 0.0} if name.lower() == 'earth' else {'distance_from_sun_m': -10.0, 'angular_position_rad': math.pi / 2} if name.lower() == 'mars' else None
    ...
    mock_hohmann_delta_v.assert_called_once_with(planner.mu_sun, ephemeris_earth_distance, static_mars_distance)
    mock_hohmann_time_of_flight.assert_called_once_with(planner.mu_sun, ephemeris_earth_distance, static_mars_distance)
    mock_lambert_transfer.assert_not_called()
...
def test_plan_trajectory_hohmann_non_existent_departure_body_with_date(
    ...
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 2.30e11, 'angular_position_rad': math.pi / 2} if name.lower() == 'mars' else None
    ...
    with patch('builtins.print'): # Suppress warning for this specific test as the overall failure is what we check
        result = planner.plan_trajectory("NonExistentPlanet", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
    assert result["success"] is False
    assert "Could not find data for departure body 'NonExistentPlanet'." in result["error"]
    mock_ephemeris_get_heliocentric_state.assert_called_once_with('NonExistentPlanet', departure_date_obj) # ephemeris tried first
...
def test_plan_trajectory_hohmann_non_existent_arrival_body_with_date(
    ...
    # Ensure ephemeris doesn't have data for 'NonExistentPlanet'
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 1.50e11, 'angular_position_rad': 0.0} if name.lower() == 'earth' else None

    # Ensure celestial data also doesn't have data for 'NonExistentPlanet' (fixture default)

    with patch('builtins.print'): # Suppress warning for this specific test
        result = planner.plan_trajectory("Earth", "NonExistentPlanet", trajectory_type="Hohmann", departure_date=departure_date_str)
    assert result["success"] is False
    assert "Could not find data for arrival body 'NonExistentPlanet'." in result["error"]
    mock_ephemeris_get_heliocentric_state.assert_has_calls([
        call('Earth', departure_date_obj),
        call('NonExistentPlanet', departure_date_obj)
    ], any_order=True)

def test_plan_trajectory_hohmann_orbital_mechanics_value_error_with_date(mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state):
    """
    Test planning when orbital_mechanics functions raise a ValueError for Hohmann transfer, with date.
    """
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    mock_hohmann_delta_v.side_effect = ValueError("Test orbital calculation error")
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
...
def test_plan_trajectory_hohmann_orbital_mechanics_unexpected_error_with_date(mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state):
    """
    Test planning when orbital_mechanics functions raise an unexpected error for Hohmann transfer, with date.
    """
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    mock_hohmann_time_of_flight.side_effect = TypeError("Unexpected type error in orbital mechanics")
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
...
def test_plan_trajectory_hohmann_with_fractional_travel_time_with_date(mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state):
    """
    Test planning with fractional travel time to ensure correct H:M:S formatting for Hohmann transfer, with date.
    """
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    mock_hohmann_time_of_flight.return_value = 100 * 24 * 3600 + 3 * 3600 + 30 * 60 + 15 # 100 days, 3h 30m 15s
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Hohmann", departure_date=departure_date_str)
...
def test_plan_trajectory_default_type_hohmann_with_date(
    ...
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    result = planner.plan_trajectory("Earth", "Mars", departure_date=departure_date_str) # No trajectory_type specified
...
def test_plan_trajectory_unsupported_type_with_date(mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state):
    """
    Test plan_trajectory with an unsupported trajectory type, with a date.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Unsupported", departure_date=departure_date_str)
    assert result["success"] is False
    ...
    mock_ephemeris_get_heliocentric_state.assert_not_called() # No ephemeris calls for unsupported types

def test_plan_trajectory_invalid_type_non_string_with_date(mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state):
    """
    Test plan_trajectory with a non-string trajectory type, with a date.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type=123, departure_date=departure_date_str)
    assert result["success"] is False
    assert "Trajectory type must be a non-empty string." in result["error"]
    mock_ephemeris_get_heliocentric_state.assert_not_called() # No ephemeris calls for invalid types

# --- NEW TESTS for _plan_direct_transfer method ---

def test_plan_trajectory_direct_transfer_success_earth_mars_with_ephemeris(
    mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state
):
    """
    Test successful planning of an Earth to Mars direct transfer using plan_trajectory
    with a departure_date, leveraging ephemeris data for positions.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)
    ...
    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Direct Transfer", departure_date=departure_date_str)

    assert result["success"] is True
    assert result["departure_body"] == "Earth"
    assert result["arrival_body"] == "Mars"
    assert result["transfer_type"] == "Direct Transfer"
    assert result["total_delta_v_mps"] == pytest.approx(5000.0) # From mock_lambert_transfer
    assert result["travel_time_days"] == pytest.approx(50.0) # From mock_lambert_transfer
    assert result["travel_time_h_m_s"] == "50 days, 00 hours, 00 minutes, 00 seconds"
    assert result["error"] is None
    ...
    # Verify Hohmann transfer functions were NOT called
    mock_hohmann_delta_v.assert_not_called()
    mock_hohmann_time_of_flight.assert_not_called()

    # Verify Lambert transfer function was called with the correct arguments
    earth_state = mock_ephemeris_get_heliocentric_state.side_effect('Earth', departure_date_obj)
    mars_state = mock_ephemeris_get_heliocentric_state.side_effect('Mars', departure_date_obj)

    r_earth = earth_state['distance_from_sun_m']
    r_mars = mars_state['distance_from_sun_m']
    delta_theta = abs(mars_state['angular_position_rad'] - earth_state['angular_position_rad'])

    mock_lambert_transfer.assert_called_once_with(
        planner.mu_sun,
        r_earth,
        r_mars,
        delta_theta
...
def test_plan_trajectory_direct_transfer_input_validation_non_existent_body(
    mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state
):
    """
    Test direct transfer planning with a non-existent arrival body.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)

    # Configure ephemeris mock to return data for Earth, but None for 'NonExistentPlanet'
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 1.50e11, 'angular_position_rad': 0.0} if name.lower() == 'earth' else None

    with patch('builtins.print'): # Suppress any warnings from fallback attempts
        result = planner.plan_trajectory("Earth", "NonExistentPlanet", trajectory_type="Direct Transfer", departure_date=departure_date_str)
...
def test_plan_trajectory_direct_transfer_orbital_mechanics_value_error(
    mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state
):
    """
    Test direct transfer planning when orbital_mechanics.calculate_lambert_transfer raises a ValueError.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    mock_lambert_transfer.side_effect = ValueError("Lambert calculation impossible")

    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Direct Transfer", departure_date=departure_date_str)
    
    assert result["success"] is False
    assert "Orbital mechanics calculation error: Lambert calculation impossible" in result["error"]
    mock_hohmann_delta_v.assert_not_called()
    mock_hohmann_time_of_flight.assert_not_called()
    mock_lambert_transfer.assert_called_once() # Verify it was called before raising error


def test_plan_trajectory_direct_transfer_orbital_mechanics_unexpected_error(
    mock_celestial_data_for_planner, mock_orbital_mechanics, mock_ephemeris_get_heliocentric_state
):
    """
    Test direct transfer planning when orbital_mechanics.calculate_lambert_transfer raises an unexpected error.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    
    mock_hohmann_delta_v, mock_hohmann_time_of_flight, mock_lambert_transfer = mock_orbital_mechanics
    mock_lambert_transfer.side_effect = TypeError("Unexpected argument type")

    result = planner.plan_trajectory("Earth", "Mars", trajectory_type="Direct Transfer", departure_date=departure_date_str)
    
    assert result["success"] is False
    assert "An unexpected error occurred during Direct Transfer calculation: Unexpected argument type" in result["error"]
    mock_hohmann_delta_v.assert_not_called()
    mock_hohmann_time_of_flight.assert_not_called()
    mock_lambert_transfer.assert_called_once()


def test_plan_trajectory_direct_transfer_ephemeris_and_static_fail_departure(
    mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state
):
    """
    Test direct transfer planning when ephemeris fails for departure, AND static data is also missing.
    Expected: Failure.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
...
def test_plan_trajectory_direct_transfer_ephemeris_missing_angular_position(
    mock_celestial_data_for_planner, mock_ephemeris_get_heliocentric_state
):
    """
    Test direct transfer planning when ephemeris returns data but without 'angular_position_rad' for departure.
    Expected: Failure, as angular position is critical for direct transfers.
    """
    planner = TrajectoryPlanner()
    departure_date_str = "2025-01-01"
    departure_date_obj = _parse_date_string(departure_date_str)

    # Configure ephemeris mock to return distance but no angle for Earth
    mock_ephemeris_get_heliocentric_state.side_effect = lambda name, date: \
        {'distance_from_sun_m': 1.50e11} if name.lower() == 'earth' else \
        {'distance_from_sun_m': 2.30e11, 'angular_position_rad': math.pi / 2} if name.lower() == 'mars' else None
    ...
    assert result["success"] is False
    assert "Required 'angular_position_rad' missing from ephemeris data for 'Earth'." in result["error"]
    mock_ephemeris_get_heliocentric_state.assert_has_calls([
        call('Earth', departure_date_obj),
        call('Mars', departure_date_obj)
    ], any_order=True)
    mock_print.assert_not_called() # No fallback warning, as it's a critical missing key, not just no data.